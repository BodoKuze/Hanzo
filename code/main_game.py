...
class Game:
    def __init__(self,master,width,height) -> None:
        ...
        self.p = pygame
        self.player = Player(master,self.p.K_w,self.p.K_s,self.p.K_a,self.p.K_d,self.p.K_SPACE,self.p.K_k,width//2,500)   
        self.delta_time = 0
        ...
        self.platforms_hit_box_list = [Construction(master,0,10,16,1,a,[0]*16,self.camera)]
    ...
    def update_main(self,e):
        ...
        self.camera.update(self.player.hit_box,self.delta_time)
    ...
    def update_enteties(self,dt,e):
        self.player.update(self.delta_time,self.platforms_hit_box_list,None,e,self.camera)